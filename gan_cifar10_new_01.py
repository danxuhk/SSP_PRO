...
def generate_image(frame, netG):
    ...
    lib.save_images.save_images(samples, './results_new/cifar10_1/samples_{}.jpg'.format(frame))
...
eval_gen, dev_gen = lib.cifar10.load(BATCH_SIZE, DATA_DIR, Scores, 1, 1)
train_gen, dev_gen = lib.cifar10.load(BATCH_SIZE, DATA_DIR, Scores, 1, 0)
def inf_train_gen():
    while True:
...
n_iter_per_SP_iter = np.round(tot_SP_iter / n_SP_iter)
...
for iteration in xrange(ITERS):
    ...
    if iteration >= SP_iter_begin and iteration < SP_iter_end:
        ...
        if (iteration - SP_iter_begin) % n_iter_per_SP_iter == 0:
            Scores = []
    ...
    lib.plot.plot('./results_new/cifar10_1/train disc cost', D_cost.cpu().data.numpy())
    lib.plot.plot('./results_new/cifar10_1/time', time.time() - start_time)
    lib.plot.plot('./results_new/cifar10_1/train gen cost', G_cost.cpu().data.numpy())
    lib.plot.plot('./results_new/cifar10_1/wasserstein distance', Wasserstein_D.cpu().data.numpy())

    # Calculate inception score every 1K iters
    if True and iteration % 50000 == 49999:
        inception_score = get_inception_score(netG)
        lib.plot.plot('./results_new/cifar10_1/inception score', inception_score[0])
    ...
    if iteration % 100 == 99:
        ...
        lib.plot.plot('./results_new/cifar10_1/dev disc cost', np.mean(dev_disc_costs))
